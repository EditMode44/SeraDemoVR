...
public class GrabHandPose : MonoBehaviour
{
    ...
    public IEnumerator SetHandDataRoutine(HandData h, Vector3 newPosition, Quaternion newRotation, Quaternion[] newBonesRotation, Vector3 startingPosition, Quaternion startingRotation, Quaternion[] startingBoneRotations)
    {
        float timer = 0;
        ...
        while (timer < poseTransitionDuration)
        {
            ...
            for (int i = 0; i < newBonesRotation.Length; i++)
            {
                h.fingerBones[i].localRotation = Quaternion.Lerp(startingBoneRotations[i], newBonesRotation[i], timer / poseTransitionDuration);
            }
    ...
    public void MirrorPose(HandData poseToMirror, HandData poseUsedToMirror)
    {
        ...
        Quaternion mirorredQuaternion = poseUsedToMirror.root.localRotation;
        mirorredQuaternion.y *= -1f;
        mirorredQuaternion.z *= -1f;

        poseToMirror.root.localPosition = mirroredPosition;
        poseToMirror.root.localRotation = mirorredQuaternion;
